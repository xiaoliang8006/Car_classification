...
# build network
tf_input = tf.placeholder(tf.float32, [None, 25], "input") #行数不限，列数25
tfx = tf_input[:, :21]
tfy = tf_input[:, 21:]

l1 = tf.layers.dense(tfx, 128, tf.nn.relu, name="l1") #128个神经元
...
sess = tf.Session()
sess.run(tf.group(tf.global_variables_initializer(), tf.local_variables_initializer())) #accuracy是一个local的变量
